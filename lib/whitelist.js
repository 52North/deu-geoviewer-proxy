...
WhitelistManager.prototype.fetch = function(format) {
    ...
    return new Bluebird(function(resolve, reject) {
        // the new API seems to lowercase the format in queries
        var options = {
            ...
            qs: {
                ...
                page: 0,
                facets: '{"format":["' + format.toLowerCase() + '"]}',
        ...
        function onResponse(body) {
            if (body && body.success && body.result) {
                ...
                if (body.result.count > ((options.qs.page * options.qs.limit) + options.qs.limit)) {
                    options.qs.page += 1;
        ...
        function extractDomains(body, domains) {
            ...
            body.result.results.forEach(function(p) {
                ...
                if (p && p.distributions && p.distributions.length > 0) {
                    p.distributions.forEach(function(d) {
                        if (d.format && d.format.id && d.format.id.toUpperCase() === format.toUpperCase()) {
                            if (d.access_url) urls.push(d.access_url);
                        }
                    });
                }
