...
WhitelistManager.prototype.refresh = function() {
  ...
  */
  ...
  var fetches = [
    ...
    this.fetch("wms"),
    this.fetch("WMS_SRVC"),
    this.fetch("fiware_cb"),
    this.fetch("FIWARE_CB"),
    this.fetch("FIWARE-CB"),
    this.fetch("view"),
    this.fetch("VIEW"),
  ];
  

  Bluebird.all(fetches).then(function(domains) {
      ...
      results.forEach(function(p) {
        if (p.distributions) {
          p.distributions.forEach(function(distribution) {
            if (
              distribution.format &&
              (distribution.download_url || distribution.access_url) &&
              format.toLowerCase() == distribution.format.id.toLowerCase()
            ) {
              var urls = [];
              if (distribution.download_url) {
                if (Array.isArray(distribution.download_url)) {
                  Array.prototype.push.apply(urls, distribution.download_url);
                } else {
                  urls.push(distribution.download_url);
                }
              }
              if (distribution.access_url) {
                if (Array.isArray(distribution.access_url)) {
                  Array.prototype.push.apply(urls, distribution.access_url);
                } else {
              ...
              urls.forEach(function(url) {
                try {
                  ...
                  if (!domain) return;
                  domain = domain.toLowerCase().replace(/:[0-9]+$/, "")
                  if (!domains[domain]) {
