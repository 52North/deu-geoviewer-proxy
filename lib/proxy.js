...
Proxy.prototype.getTarget = function(req) {
  ...
  return URL.parse(decodeURIComponent(target));
};
...
Proxy.prototype.accepts = function(target) {
  ...
  return this.whitelist.contains(target.host.replace(/:[0-9]+$/, ""));
};
...
Proxy.prototype.createRequest = function(target, req, cb) {
  var options = {};
  ...
  if (options.method && options.method.toString().toLowerCase() === "get") {
  ...
  if (target.protocol === "https:") {
    return https.request(URL.format(target), options, cb);
  } else {
    return http.request(URL.format(target), options, cb);
  }
};
...
Proxy.prototype.proxyRequest = function(target, req, res) {
  ...
  var preq = this.createRequest(target, req, function onResponse(pres) {
    ...
    if (pres.headers.location) {
      var redirectLocation = pres.headers.location;
      //send this to the whitelist, should be in it
      var domain = URL.parse(redirectLocation);
